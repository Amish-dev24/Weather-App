...
const WeatherAlert = ({ apiKey, city }) => {
    ...
    useEffect(() => {
        ...
        fetchAlerts();
    }, [city, apiKey]); // Re-fetch when city or apiKey changes
        ...
        <div className="weather-alerts-container">
            ...
            {coordinates && (
                <p className="location-info">
                    Location: {coordinates.lat.toFixed(4)}°N, {coordinates.lon.toFixed(4)}°E
                </p>
            )}
            {alerts.length > 0 ? (
                ...
                <p className="no-alerts">No weather alerts at this time for {locationName || city}.</p>
            )}
        </div>
    );
};
